...
import argparse, binascii, code, glob, os, os.path, unittest
...
def new_test(domain, qtype, address="8.8.8.8", port=53, nodig=False, dnssec=False):
    ...
    if not nodig:
        if dnssec:
            dig = getoutput(
                "dig +qr +dnssec -p %d %s %s @%s" % (port, domain, qtype, address)
            )
        else:
            dig = getoutput(
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--unittest", action="store_true", default=True, help="Run unit tests"
    )
    p.add_argument(
        "--verbose", action="store_true", default=False, help="Verbose unit test output"
    )
    p.add_argument(
        "--failfast",
        action="store_true",
